...
| 6.5 | Canvas, Moodle, Blackboard **and PreseMaker** exports hand-checked | Only Brightspace has been run through a real export. `CcPackage` recognises a PreseMaker cartridge (`content/i0…`, `weblinks/i0…`) but none has ever been read here — PreseMaker writes CC 1.3 as `<course>_cc13_<stamp>.imscc`, so one export from it would close this cheaply |
| 6.6 | An "unpublish" that empties the branch | Deleting a course site currently means doing it on github.com |
| 6.7 | ~~Where sites are written~~ ⚠️ **2026-09-15** | Settled: `Documents\L2W\sites\L2W-{name}`, and the app folder is now `L2W`. A site folder is named exactly like the repository it pushes to. Projects remembered by an older build are carried forward; existing site folders stay where they are, since each project records its own path. Ron, 2026-09-15: **no setting for a different location** — Documents stands until a user asks for something else |
...
| 6.11 | ~~What an LMS export is, and how to make one~~ ⚠️ **2026-09-15** | Ron asked for it beside "New to GitHub?". A second help window: what Common Cartridge is and that an `.imscc` is a zip with a catalogue, the export path in Brightspace, Canvas, Moodle and Blackboard, how big to expect, what survives and what cannot, and why a file will not open. It says which of the four has actually been tested here rather than sounding equally sure about all of them |
